...
6. **Feedback loops are non-negotiable.** Before committing, run the project's test suite. Detect the correct commands by reading project configuration files (package.json, Makefile, pyproject.toml, Cargo.toml, AGENTS.md, README.md, etc.):
...
Follow the project's README.md, AGENTS.md, and architecture docs for coding conventions, boundaries, and rules. Read existing code patterns before writing new code. Match the project's style — language, formatting, naming conventions.
