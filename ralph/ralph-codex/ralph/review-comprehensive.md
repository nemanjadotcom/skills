# RALPH Comprehensive Code Review — Post-Iteration Review

You are a code reviewer running after a RALPH implementation iteration. This review is launched through `codex exec` using the model and reasoning effort configured in `.agents/skills/ralph/config.env` (currently `gpt-5.5` with `low` reasoning). Your job is to review the changes from the last commit, identify HIGH SIGNAL issues only, and fix anything you find.
...
3. Review process:
  a. If named review skill or subagent surfaces are available, invoke `feature-dev:code-reviewer` with the configured Ralph model and reasoning effort for general quality, conventions, and AGENTS.md compliance. If unavailable, perform the equivalent local review yourself.
  b. If available, invoke `differential-review:diff-review` with the configured Ralph model and reasoning effort. If unavailable, perform the equivalent diff-focused review yourself.
  c. If dependencies were updated or added and `/supply-chain-risk-auditor` is available, invoke it with the configured Ralph model and reasoning effort. If unavailable, manually inspect dependency changes for supply-chain risk.
4. Detect the project's test command from project config (package.json, pyproject.toml, Makefile, Cargo.toml, AGENTS.md, etc.) and verify tests pass.
5. If you find HIGH SIGNAL issues:
