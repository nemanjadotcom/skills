...
3. Run the review pass:
   a. If named review skill or subagent surfaces are available, invoke `feature-dev:code-reviewer` with the configured Ralph model and reasoning effort; otherwise perform the equivalent local quality/convention review yourself.
   b. If available, invoke `differential-review:diff-review` with the configured Ralph model and reasoning effort; otherwise perform the equivalent diff-focused review yourself.
   c. If dependencies were updated or added and `/supply-chain-risk-auditor` is available, invoke it with the configured Ralph model and reasoning effort; otherwise manually inspect dependency changes for supply-chain risk.
